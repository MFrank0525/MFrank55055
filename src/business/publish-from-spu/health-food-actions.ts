...
export async function checkHealthFunctionOptionOnPage(
  ...
  if (selectedBefore.some((value) => normalizeDomText(value).includes(normalizeDomText(optionText)))) {
    return {
      action: "check_health_function",
      label: "保健功能",
      expectedOption: optionText,
  ...
  await search.fill(optionText, { timeout: 5000 });
  const titles = page.locator(".ecom-g-select-tree-title").filter({ hasText: optionText });
  ...
  for (let index = 0; index < titleCount; index += 1) {
    ...
    if (
      ...
      normalizeDomText(await title.innerText({ timeout: 500 }).catch(() => "")) === normalizeDomText(optionText)
    ) {
      exactTitle = title;
      break;
    }
  ...
  const checked = selected.some((value) => normalizeDomText(value).includes(normalizeDomText(optionText)));
  return {
    action: "check_health_function",
    label: "保健功能",
    expectedOption: optionText,
