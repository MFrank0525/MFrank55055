...
assert.equal(
  isSpecTemplateSmartFillUploadModeText("智能填写助手 切换手动填写 点击 或 拖动 文件到虚线框内上传"),
  true
);
assert.equal(
  isSpecTemplateSmartFillUploadModeText("智能填写助手\n切换手动填写\n点击或拖动文件到虚线框内上传"),
  true,
  "Smart-fill mode detection must tolerate the compact wording rendered by some shops"
);
...
assert.match(
  publishRulesSource,
  /function isSpecTemplateSmartFillUploadModeText[\s\S]*智能填写助手[\s\S]*切换手动填写[\s\S]*点击或拖动文件到虚线框内上传/,
...
assert.match(
  publishSource,
  /async function clickSwitchManualSpecEntryMode[\s\S]*智能填写助手[\s\S]*切换手动填写[\s\S]*点击或拖动文件到虚线框内上传[\s\S]*querySelectorAll\("button, \[role='button'\], a, body \*"\)/,
