...
- 飞书与抖店浏览器分别承担数据和发布边界；主图生成只允许唯一 canonical OpenAI-compatible `gpt-image-2`、`videos-base64`、精确接口 `/v1/videos`，并通过 Base64 data URL 元数据传入当前白底参考图。
- 每个节点必须声明输入、输出、成功条件和失败条件；主图节点不得增加替代 provider、模型、模式、端点或兼容入口。
