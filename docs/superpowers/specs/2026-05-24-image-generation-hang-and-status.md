...
恢复时又暴露出多任务 run 的断点缺口：单任务 resume 会重新创建 `image-001`，但原失败产物保存在旧 run 的 `image-002` 目录，导致恢复找不到已生成的 Word 提示词。
...
6. 多任务 run 中间任务失败后恢复，必须保留原 `taskId`，否则恢复动作会读错任务产物目录。
...
6. resume job 保存 `resumeTaskId`，运行状态创建后应用该 task id。
...
4. 回归测试覆盖 resume task id 保留。
5. `npm run build && node scripts/test-progress-state.mjs`
6. `npm run rules:check`
7. `npm run doctor:auto-listing`
