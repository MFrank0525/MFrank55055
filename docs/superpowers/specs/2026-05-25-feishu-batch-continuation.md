...
3. 不同批次之间，即使产品、SPU 或白底图附件名相同，也必须当作新任务继续执行。
4. 白底图和资质图的本地路径必须是 `recordId + 附件身份` 级唯一；同批文、同通用名称、同用户认知名不能作为包装相同的依据。不同飞书记录共享同一个本地附件路径时，审计和生图入口必须失败，直到刷新附件生成独立路径。
5. Hermes 在一批产品全部完成后，必须刷新飞书表格素材；如果刷新后批次指纹变化且存在待处理产品，就继续启动完整上架流程。
6. Hermes 只有在当前批次已完成、刷新飞书后没有发现新批次待处理产品时，才停止上架项目。
