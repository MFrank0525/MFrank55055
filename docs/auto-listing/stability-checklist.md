...
37. 图片供应商返回 `429/502/503/504`、`temporarily unavailable`、gateway unavailable、资源过载，或主图生成阶段出现 `fetch failed`、socket reset、timeout、`UND_ERR` 等传输失败时，规则层必须归类为外部服务可用性故障。`videos-base64` 已受理任务在状态轮询中长期只有 `queued 0`/`pending 0` 并触发 watchdog 时，也必须归类为外部服务等待，而不是普通快速恢复。此类故障不消耗普通流程有限恢复预算；必须保留当前飞书批次、当前产品断点、付费账本 task ID 和已验证 raw 图片。所有外部服务故障固定等待 5 分钟后自动恢复，不再指数增长。若同一已受理 task 超过 `acceptedQueueStaleMs` 仍只停留在队列心跳，必须按供应商死队列处理：保留旧 task 审计，固定 slot 标记为 `failed_after_acceptance` 后补交该 slot。权限拒绝、access forbidden、业务校验错误和可能产生不确定外部副作用的错误禁止套用此规则。
