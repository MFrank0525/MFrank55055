...
41.2 Hermes 的主动反馈必须绑定触发 `开始/继续/查询上架` 的精确平台、聊天、话题线程和消息 ID，并持久化该 origin 以跨 gateway 重启恢复；每条主动反馈必须通过 `reply_to` 直接回复该触发消息。origin 缺失时必须失败关闭，禁止遍历 channel directory、读取 home channel 或猜测历史线程。飞书发送只有同时取得成功响应和非空 `message_id` 才算送达；发送失败或缺少回执时不得推进 notice 去重状态，下一轮必须继续重试。
