...
51. 店铺真实商品规格 DOM 只出现 `添加规格类型（0/3）`、`规格预览` 而不存在 `规格模板` 时，属于店铺配置缺失，不得按页面瞬态重试。项目禁止重新引入手工创建规格值动作；必须隔离该店铺的未提交目标、继续后续店铺，并在终态消息中准确报告缺失配置。该店铺级配置缺失不得打开跨店铺熔断，也不得消耗 supervisor 的重复恢复预算。
