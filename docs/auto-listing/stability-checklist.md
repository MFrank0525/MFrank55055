...
40.1 图片固定槽熔断错误已携带有效 `retry after <ms>ms` 时，supervisor 必须按该槽位期限与通用外部服务退避两者中的较长时间调度，禁止在槽位冷却到期前固定每 10 分钟空唤醒。等待状态必须优先展示剩余倒计时、准确重试时间和固定槽位，禁止继续展示终态前的旧生图进度。运行中存在当前任务但当前 runtime 尚无发布清单时，必须优先展示当前任务状态，禁止读取同一 supervisor 日志中上一商品的发布模块。此规则只调整调度和状态展示，不得修改提示词、`requestDigest`、`promptDigest` 或付费槽位重试资格。
