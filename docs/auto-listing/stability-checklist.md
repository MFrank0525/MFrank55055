...
41.2 Hermes 的主动反馈必须绑定触发 `开始/继续/查询上架` 的精确平台、聊天和话题线程，并持久化该 origin 以跨 gateway 重启恢复；禁止遍历 channel directory 后把线程任务的进度改发到主聊天。飞书发送只有同时取得成功响应和非空 `message_id` 才算送达；发送失败或缺少回执时不得推进 notice 去重状态，下一轮必须继续重试。
