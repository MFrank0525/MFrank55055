...
40.1 图片固定槽熔断错误已携带有效 `retry after <ms>ms` 时，supervisor 必须使用该槽位剩余期限，但任何图片服务等待不得超过 3 分钟。等待状态必须优先展示剩余倒计时、准确重试时间和固定槽位，禁止继续展示终态前的旧生图进度。运行中存在当前任务但当前 runtime 尚无发布清单时，必须优先展示当前任务状态，禁止读取同一 supervisor 日志中上一商品的发布模块。此规则只调整调度和状态展示，不得修改提示词、`requestDigest`、`promptDigest` 或付费槽位重试资格。
