...
| Requirement | Implementation | Verification | Status |
| --- | --- | --- | --- |
| Distinguish multiple same-name platform brands | Query actions freeze exact-name options by stable `standard_brand_id`, verify the selected identity before and after SPU entry, and advance candidates only in that frozen order | Real `龙仕康` dropdown exposed three identities; regression covers ordering, deduplication and fail-closed identity readback | verified |
