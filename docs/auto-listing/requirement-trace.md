...
| Reconcile OTC price-row requirements without mutating specifications | The platform-prefilled OTC create page exposes one price/inventory row while the current Feishu rule requires four; creating specification rows is forbidden, and selecting one of four prices per listing is not yet an authorized rule | Four bounded live attempts all stopped before final submit with `row 2/3/4 missing`; submit state remained `not_attempted` | business decision required |
