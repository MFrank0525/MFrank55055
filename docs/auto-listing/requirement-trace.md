...
| Clear platform-prefilled main and detail images against the current Doudian DOM | Main clearing hovers the exact material preview and clicks its semantic `#icon-shanchu` action; detail clearing scopes to `商品详情`, hovers the sortable image and clicks its `iconDelete` control; both retain strict zero-count readback | Live DOM readback on run `20260809-214041`; `test-graphic-prefill-clear-rule.mjs` | implemented; verification pending |
