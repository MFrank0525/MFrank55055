...
| Clear platform-prefilled main images against the current Doudian DOM | Main-image clearing scopes to the exact `主图` field, hovers the last material preview, resolves the semantic `#icon-shanchu` control and clicks its action container before strict zero-count readback | Live DOM readback on run `20260809-214041`; `test-graphic-prefill-clear-rule.mjs` | implemented; verification pending |
