...
| Preserve actionable evidence for service-fulfillment failures | Exact field matching accepts whitespace-normalized leaf text but never fuzzy option text; failures save a service screenshot and bounded after-sales DOM diagnostics before stopping | Service-fulfillment, DOM-only, no-coordinate and module-size regressions | implemented; live verification pending |
