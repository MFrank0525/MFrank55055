...
| Remove the live specification-editor race | The first real shop proved the new safety fields and then exposed a separate async mount race: the split-editor shell was visible before its first quantity input existed. The action now gates initial controls at `1/1`, expands the rule, then gates second-stage controls at `2/2`, with bounded polling and structural diagnostics | Run `20260915-232414` first attempt failed pre-submit at quantity index 0; controlled rerun published shops 1 and 2 and advanced to shop 3; DOM sequence regression | verified |
