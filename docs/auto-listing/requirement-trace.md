...
| Preserve an exact publish resume across a pre-task browser-preflight failure | A `published` resume with exact manifest-backed pending folders outranks the freshly initialized empty task state left by preflight failure, so the first restarted child cannot fall back to title generation | Red-before-green `hasPendingPublishWork` regression in `test-progress-state-part-3.mjs`; live run `20260810-191157` exposed the stale `titles_generated` fallback before the rule fix | verified |
