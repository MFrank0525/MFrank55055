...
```text
当一个产品从第一步开始到最后上架任务完成后，只允许保留没有添加水印的主图。无水印主图必须放入以时间前缀加飞书表格本产品 `导购短标题` 命名的文件夹，并保存到：
/Users/mfrank/Desktop/FFC的文件夹/工作/001电商/2026AI主图

文件夹名称格式为 `<yyyyMMddHHmm><导购短标题>`，时间前缀精确到分钟，例如 `202605201106凡士林润唇软膏`。
...
2. 归档文件夹名称必须等于当前时间前缀加飞书表格当前产品 `导购短标题`，格式为 `<yyyyMMddHHmm><导购短标题>`。
...
1. 检查 `/Users/mfrank/Desktop/FFC的文件夹/工作/001电商/2026AI主图/<yyyyMMddHHmm><导购短标题>/` 是否存在无水印主图。
