...
34. 无法自动找回任务 ID 的 `ambiguous` 槽位永久禁止因时间经过而自动重新提交。只有通过项目自有的显式对账恢复动作，验证供应商任务 ID 和状态、确认任务未失败并写入审计原因后，才能把该槽位恢复为 `submitted`；恢复后只允许轮询同一个任务 ID。
35. 断点恢复必须按固定文件槽位识别已有和缺失图片，禁止按已有文件数量推算下一槽位。例如当前轮存在 `generated-02/03/04` 且缺少 `generated-01` 时，只能恢复或补齐第 1 张，不能重复复用第 4 张凑数量；staged 文件必须与同槽位 raw 原图一一对应。
