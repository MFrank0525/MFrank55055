...
状态文本由项目控制器按当前商品组生成：每个商品固定按 20 个待上架产品为一组，展示“当前商品、产品 x/20、店铺 y/10、飞书批次 x/y”。如果同一个运行清单里包含多个商品，状态不得把发布条目累加成 `60/60`、`80/80` 这类跨商品累计值。店铺总数必须以完整 `publish-plan.json` 或类目固定计划为准，不能用当前 `publish-manifest.json` 已触达的店铺数当分母。状态文本必须按当前业务阶段选择单一进度源：生图阶段才展示生图进度；进入上架发布阶段后只展示发布心跳或发布清单进度，禁止夹带已完成的生图 ready 信息。
...
- `非处方药`：只上架到 01-05 前五个店铺，5 份 Word、20 张主图、20 条标题；每个店铺分配 4 张水印主图；标题固定后缀为 `产品通用名称`，不再追加 `延草纲目`。
- `保健食品`：依次上架 01-10 十个店铺，5 份 Word、20 张主图、20 条标题；每个店铺分配 2 张水印主图；标题取消固定前缀和固定后缀，单条标题改为 28 个汉字。
