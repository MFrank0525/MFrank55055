...
保健食品记录还必须填写以下专用字段；医疗器械和非处方药记录可保持这些单元格为空：
...
- `保健功能`
- `规格`
- `保质期`：只填写数字，流程按原数字写入抖店，不追加单位
- `储藏条件`：填写抖店“贮存条件”下拉框中存在的完整选项文字

`保质期` 和 `储藏条件` 进入字段映射、缓存版本、批次指纹和发布前校验。保健食品任一值缺失、保质期不是正数，或储藏条件找不到完全一致的抖店下拉选项时，当前批次必须在最终发布前停止；不得回退到历史默认值。其他类目不读取也不填写这两个字段。
...
- `医疗器械`：依次上架 01-20 二十个店铺，5 份 Word、20 张主图、20 条标题；每个店铺分配 1 张水印主图。
- `非处方药`：只上架到 01-05 前五个店铺，5 份 Word、20 张主图、20 条标题；每个店铺分配 4 张水印主图。
- `保健食品`：依次上架 01-20 二十个店铺，5 份 Word、20 张主图、20 条标题；每个店铺分配 1 张水印主图。

标题类目规则：标题前半部分只由飞书 `标题关键词` 随机排列组合而成，不允许修改、替换、截断、引申或增加相近词。医疗器械、非处方药每条标题追加飞书 `标题固定后缀`，完整标题尽量接近 120 个平台字符且不得超过 120 个平台字符；保健食品每条标题不追加飞书 `标题固定后缀`，完整标题不得超过 60 个平台字符。

非处方药的标题固定后缀必须逐字符保留，尤其必须保留表示药品含量规格的专用符号 `*`；标题生成、工作簿写入、恢复和发布前校验都不得删除、替换或转义该符号。

主图提示词统一规则：每份 Word 为飞书五段式内容，依次是 `主图指令`、`产品卖点`、当前轮 `DeepSeek提示词`、`正向提示词`、`反向提示词`。项目不再使用内部固定主图模板或内部差异化提示词。

价格库存统一规则：飞书 `产品价格` 填写一个或多个正价格并按从大到小顺序排列；价格个数必须与所选规格模板在发布页生成的 SKU 行数完全一致。发布时按飞书顺序逐行一一填入，库存统一填写 2000；两边数量不一致时禁止部分填充并立即停止。
